...
platform. While single-system freshness is often solved locally with a
\texttt{MAX(updated\_at)} query, two signals have no standard at all: a portable
streaming consumer/freshness-lag metric, and a \emph{differential}
(source-relative) freshness that expresses how far an index, replica, or cache
trails its upstream---the silent failure mode behind stale retrieval-augmented
generation and lagging replicas. We address this gap with three contributions. First, we propose a
compact OpenTelemetry semantic convention for data staleness---a small set of
metrics (\texttt{data.staleness.age}, \texttt{lag},
\texttt{last\_update.timestamp}, \texttt{records.behind}, and SLA
threshold/breach signals) and attributes that
...
backends---PostgreSQL, Kafka, Redis, a schema registry, and an emulated
Kinesis---on a cloud instance, asserting that emitted metrics are numerically
correct (age equal to a known injected offset, exact consumer lag) and that
failures are made visible rather than fabricated, not merely that metrics
appear. Our aim is not a new freshness metric but the
...
\paragraph{Why now.}
The cost of missing freshness telemetry is rising as systems increasingly
\emph{act} on data automatically. Retrieval-augmented generation (RAG) answers
from a vector index that, once it lags the corpus it was built from, returns
confidently wrong results with no error or latency anomaly to alert on; online
feature stores exhibit analogous online--offline skew; and multi-agent systems
coordinate over shared state whose staleness directly degrades decisions. This
regime---autonomous consumers reacting to the most recent available
update---is precisely the setting in which AoI was
introduced~\cite{kaul2012,yates2021survey}. Freshness is therefore not a newly
invented concern but a long-studied one whose \emph{absence from standard
telemetry} has become newly expensive.
...
\begin{enumerate}[nosep]
  ...
  an Apache-2.0 Python SDK (\texttt{data-staleness-otel}, importable as
  \texttt{otel\_staleness}) built on
  \texttt{opentelemetry-python}, a zero-configuration Go OpenTelemetry Collector
  \emph{receiver} (\texttt{datastalenessreceiver}) that scrapes sources with no
  application code, and a Go Collector \emph{processor}
  (\texttt{datastalenessprocessor}) that derives age and evaluates SLAs centrally.
...
age. Our work proposes one and is structured to serve as the basis for a
new-area proposal to the OpenTelemetry Semantic Conventions working group.
...
\label{sec:design}
The reference implementation has three cooperating roles
(described below): a client-side SDK that \emph{measures} sources from
application code, a zero-configuration Collector \emph{receiver} that
\emph{scrapes} sources with no application code, and a Collector \emph{processor}
that can \emph{derive and evaluate} freshness centrally.
...
policy. For infrastructure whose code cannot be modified, the
\texttt{datastalenessreceiver} instead \emph{scrapes} the same signals directly
and emits the identical metrics with no application code. Any OTEL-compatible
backend (Prometheus, OTLP store, vendor) then stores and queries the result.

\paragraph{Why separate roles.}
...
and some infrastructure cannot be instrumented in application code at all, which
is what the zero-code receiver is for.
...
\texttt{data-staleness-otel} (import package \texttt{otel\_staleness}) is built on \texttt{opentelemetry-api/sdk}. The core is
...
swallowed and the remaining probes still report---design principle (2). An
empty or indeterminable reading (a probe that returns no timestamp) is likewise
surfaced as \texttt{data.staleness.probe.errors} rather than dropped, so a broken
source is never silently invisible.

\subsection{Collector receiver}
\texttt{datastalenessreceiver} is a zero-configuration Go metrics receiver: it
\emph{scrapes} freshness on a schedule from YAML alone, with no application code.
Source types cover SQL (\texttt{MAX(ts)} or a custom watermark/load-audit query),
Kafka and Amazon Kinesis (newest-record time and consumer-offset lag, per
partition/shard), files (mtime), HTTP (\texttt{Last-Modified} or an epoch body),
Confluent Schema Registry and DB-migration version drift, and a static source for
testing. Every failure or indeterminable measurement (empty table, NULL
\texttt{MAX()}, timeout, unparseable value, empty partition) is surfaced as
\texttt{data.staleness.probe.errors} with a specific \texttt{error.type}, never a
fabricated age. It emits the identical metric vocabulary as the SDK, so the two
producers are interchangeable at the backend.

\subsection{Collector processor}
\texttt{datastalenessprocessor} is a Go metrics processor (Collector API
...
ran in a containerized Linux environment (Python 3.10, Go 1.25, 2 vCPU); all
benchmarks, unit suites, and the end-to-end integration validation are scripted
and reproducible from the released repository.

\subsection{Correctness}
All three components ship with unit tests asserting the convention's semantics:
...
Kafka lag; method/attribute tagging; and probe fault isolation (an empty or
failed measurement surfaces as \texttt{probe.errors}, never a fabricated age).
The Python SDK (54 tests), the Go receiver (42 tests), and the Go processor
(9 tests) all pass, alongside a language-agnostic \emph{conformance} suite that
replays shared vectors so the numbers are not one library's opinion.
Representative invariants: \(age = t_{now}-t_e\); \(age \geq 0\); and
\(\texttt{breached}=\mathbb{1}[age > \textit{threshold}]\). Beyond unit tests, an
integration suite stands up real Postgres, Kafka, Redis, a Schema Registry, and
an emulated Kinesis (LocalStack) on a cloud instance and asserts the emitted
metrics are numerically correct---age equal to a known injected offset and exact
consumer lag---and that stopping a source makes \texttt{probe.errors} visible
...
\label{sec:limits}
Our evaluation establishes correctness and overhead---including numeric
correctness against real backends---but not breach-detection \emph{quality} on
production workloads; a natural next step is a field study comparing
...
intended path to adoption is a semantic-conventions proposal and community
review, during which names may change. Additional first-party probes and source types (BigQuery and Snowflake
information-schema freshness, GCS, lakehouse snapshot age) are straightforward
extensions of the existing SDK and receiver.
...
practice of data observability, and we released a reference implementation---a Python SDK, a zero-configuration
Collector receiver, and a Collector processor---that is correct and adds
negligible overhead. By making data age a portable, queryable telemetry signal alongside
...
The semantic-convention specification, a machine-readable Weaver model, a
language-agnostic conformance suite, the Python SDK (\texttt{data-staleness-otel}),
the Go Collector receiver (\texttt{datastalenessreceiver}) and processor
(\texttt{datastalenessprocessor}), the integration-validation suite, demo
deployments, and benchmarks are released as open source under Apache-2.0 at
\url{https://github.com/anirudhrajreliability/otel-data-staleness}. The Python
package is installable via \texttt{pip install data-staleness-otel}; all
benchmarks and validations reported here are reproducible from the repository.
